      ...
      {currentLocation !== "/communities" && (
        <div>
          <div className="flex items-end justify-between mb-4">
            <h5 className="font-semibold text-sm mb-4 md:mb-0">Suggested Communities</h5>
            {remainingCount > 0 && (
              <Link
                className="flex relative items-center text-sm font-medium text-primary mr-4 md:mr-0 md:block"
                to="/communities"
              >
                See all
          ...
          <ul className="flex flex-col gap-3 ">
            {visibleCommunities?.map((community) => (
              <li
                key={community._id}
                className="community-item flex items-center justify-between bg-white px-2 py-1 rounded-md"
              >
                ...
                <button
                  ...
                  className="rigthbar-btn text-primary border border-dashed border-blue-500
                        hover:bg-primary 
      ...
      <ul className="flex flex-col gap-3">
        {recommendedUsers?.length > 0 &&
          recommendedUsers.map((user) => (
            <li
              key={user._id}
              className="overflow-hidden whitespace-nowrap community-item flex justify-between items-center gap-5 bg-white shadow-2xl shadow-[#f2f5fc]  border border-slate-100 px-2 py-1 rounded-lg "
            >
              ...
              <button
                ...
                className="rigthbar-btn users text-primary border border-dashed border-blue-500 hover:bg-primary rounded-md py-1 px-2 text-sm font-semibold group transition duration-300"
              >
